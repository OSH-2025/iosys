...
# 构建图形索引,PropertyGraphIndex 使用 kg_extractor 从文本中提取实体和关系，并将它们作为节点和边缘存储在 GraphRAGStore 中。
import re
...
# 查询图表初始化 GraphRAGQueryEngine 以查询处理后的数据。提交查询后，引擎会从 GraphRAGStore 中检索相关的社区摘要。
# 对于每个摘要，它使用 LLM 通过 generate_answer_from_summary 方法生成与查询相关的特定答案。
