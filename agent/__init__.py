...
class IOSYSAgent:
    ...
    def _get_or_create_session(
        ...
        if session_id not in self.sessions:
            self.sessions[session_id] = [
                {
                    "role": "system",
                    "content": "你是一个文件管理助手，可以帮助用户进行各种文件操作。根据用户的需求，选择合适的工具来完成任务。如果用户的要求比较复杂, 你需要将任务拆分成多个步骤，并使用合适的工具来完成每个步骤。注意, 如果用户要求你写一份报告, 一定要确保用户提供的数据(例如表格)都正确地包含在了报告中, 且你需要注意数学公式尽量使用 LaTeX 语法来书写。",
                },
            ]
    ...
    async def process_command(self, user_input: str, session_id: str) -> Dict[str, Any]:
        ...
        self.logger.info(f"处理结果 (会话 {session_id}): {json.dumps(result)}")
        return dict(result)
                            ...
                            history.append(
                                {
                                    ...
                                    "content": json.dumps(result_item["result"]),
                                }
                            )
