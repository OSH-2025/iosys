...
- [2. 系统架构与实现](#2-系统架构与实现)
  - [2.1. 总体架构](#21-总体架构)
  - [2.2. 核心模块讲解](#22-核心模块讲解)
    - [2.2.1. Agent 模块与交互协议](#221-agent-模块与交互协议)
    - [2.2.2. File System 模块以及分布式部署](#222-file-system-模块以及分布式部署)
...
- **双重实现**:
  ...
  - **JFS**: 我们在 IOSYS 中对接了 **JuiceFS** 分布式文件系统。JuiceFS 是一款云原生存储解决方案，我们将其与阿里云 OSS 的结合，实现了系统的跨平台、高可用和易于二次开发的特性。部分效果如下:
    ...
    ![alt text](./assets/jfsCloudService.png)
    ![alt text](./assets/aliyunOss.png)
...
![alt text](./assets/parser-demo.png)
...
- **向量索引 (Vector Indexing) 与检索功能**:
  ...
  - **架构**:
    ...
    - **检索**: 用户查询通过 LlamaIndex 在 Qdrant 中进行向量相似度搜索，召回最相关的文本片段，作为上下文提供给 LLM 生成最终答案。我们使用了 openai 提供的 `text-embedding-ada-002` 模型进行 "embedding", 进而实现文件的语义检索功能.
