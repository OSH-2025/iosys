...
近年来，AI，尤其是 LLM 实现了快速发展和广泛应用。从 Chat bot 到 AI Agent，LLM 的应用更加智能化和自主化，因此也更加需要与操作系统交互的能力。在这种背景下，AIOS 应运而生。AIOS 是一个专门为 AI Agent 和 LLM 设计的操作系统，旨在提供高效的资源管理、调度和上下文维护能力。其架构如下 ([mei2024aiosllmagentoperating](https://arxiv.org/abs/2403.16971))：

![AIOS Architecture](../assets/research/image-1.png)

这张图展示了 AIOS 的分层系统架构，旨在将 AI Agent 的能力与操作系统功能融合，其结构自上而下分为三层：

1.  **应用层 (Application Layer)**:

    - **Agent Application**: 这是顶层，包含了各类具体的 AI 智能体应用（如旅行 Agent、推荐 Agent、编码 Agent、数学 Agent、叙事 Agent 等），均面向特定任务或领域。
    - **AIOS-Agent SDK (Software Development Kit)**: 这是关键的中间件/接口层，直接服务于上层 Agent 应用，其主要作用是：
      - 接收来自 Agent 应用的请求。
      - **区分请求类型：**
        - **LLM 相关查询 (LLM Related Queries)**: 例如推理与规划、工具解析、代码生成等需要大型语言模型支持的请求，将被导发至 AIOS Kernel 处理。
        - **非 LLM 相关查询 (Non-LLM Related Queries)**: 如传统的文件操作、网络请求、键盘/鼠标输入等，则交由标准 OS Kernel 处理。
...
Manus 相比现有的 Agent 有着更高的完成度，但本质上仍是一个 Agent。它拥有较多工具，并在虚拟的环境中操作。它可以独立自主地完成较复杂的要求，如查询和总结信息、编写代码处理数据和文档等。
