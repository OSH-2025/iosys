...
2. npl, batch_size
    ...
    这两者对于输出质量几乎无影响，但在推理速度和显存占用上有较大影响，正如上面两节所提到的，建议选择均衡的参数组合，在 ngl=30, batch_size=128 时最为均衡。
...
可以看到使用 RPC 比本地慢了约 2 tokens/s，且通过性能检测器可以看到 RPC 部署时 12 个核没有全部达到 100% 利用率，而 LOCAL 部署时达到了几乎每核均在 100% 。
